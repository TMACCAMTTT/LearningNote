...
class TestVPTree(unittest.TestCase):
    ...
    def test_single_nearest_neighbor(self):
        ...
        tree = vptree.VPTree(points, euclidean)  # 建树

        nearest = tree.get_nearest_neighbor(query)  # 在VP树中找到的查询点的最近邻
        bf_nearest = brute_force[0]  # 暴力解法得到的查询点的最近邻
...
def brute_force_solution(n, dim, query, dist=euclidean):  # 暴力解法
    points = np.random.randn(n, dim)
    brute_force = [(dist(query, point), point) for point in points]
    brute_force.sort()

    return points, brute_force  # 返回随机生成的数据点以及一个列表：每个元素是(某个数据点到查询点的距离,某个数据点)
